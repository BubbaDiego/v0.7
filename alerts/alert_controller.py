...
class AlertController:
    ...
    def create_travel_percent_alerts(self):
        ...
        for pos in positions:
            if not pos.get("alert_reference_id"):
                ...
                # Use the "low" threshold from travel_config as the trigger value (adjust as needed)
                ...
                if self.create_alert(dummy_alert):
                    ...
                    # Update the position's alert_reference_id with the alert's ID
                    try:
    ...
    def create_all_alerts(self):
        """
        Calls create_price_alerts() and create_travel_percent_alerts() and returns the combined results.
        """
        price_alerts = self.create_price_alerts()
        travel_alerts = self.create_travel_percent_alerts()
        return price_alerts + travel_alerts
