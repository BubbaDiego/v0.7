...
def dydx_data():
    try:
        ...
        # First attempt the general subaccounts endpoint
        subaccounts = client.get_subaccounts(wallet_address)
        if not subaccounts:
            current_app.logger.debug("No subaccounts found; trying specific subaccount endpoint.")
            subaccount = client.get_subaccount(wallet_address, subaccount_number)
            subaccounts = [subaccount] if subaccount else []

        positions = client.get_perpetual_positions(wallet_address, subaccount_number)
        return render_template("dydx.html", subaccounts=subaccounts, positions=positions)
    except Exception as e:
...
@positions_bp.route("/update_positions", methods=["GET", "POST"])
def update_positions():
    """
    New endpoint that updates positions from both Jupiter and dYdX.
    This endpoint will:
      1. Delete existing Jupiter positions.
      2. Update Jupiter positions.
      3. Update dYdX positions.
      4. Trigger a price update.
      5. Run manual alert checks.
      6. Update last update timestamps.
      7. Record a snapshot.
      8. Emit a SocketIO update event.
      9. Log the operation.
    """
    source = request.args.get("source") or "user"
    if source != "monitor":
        source = "user"
    logger.debug(f"Update positions called with source: {source}")
    print(f"[DEBUG] Update positions route triggered with source: {source}")
    ...
    try:
        ...
        jupiter_result = PositionService.update_jupiter_positions(DB_PATH)
        logger.debug(f"Jupiter update result: {jupiter_result}")
        print(f"[DEBUG] Jupiter positions update result: {jupiter_result}")
        if "error" in jupiter_result:
            logger.error("Error during Jupiter positions update: " + str(jupiter_result))
            print("[ERROR] Error during Jupiter positions update:", jupiter_result)
            return jsonify(jupiter_result), 500
    ...
    # Step 3: Update dYdX positions
    try:
        logger.debug("Step 3: Updating dYdX positions via PositionService...")
        dydx_result = PositionService.update_dydx_positions(DB_PATH)
        logger.debug(f"dYdX update result: {dydx_result}")
        print(f"[DEBUG] dYdX positions update result: {dydx_result}")
        if "error" in dydx_result:
            logger.error("Error during dYdX positions update: " + str(dydx_result))
            print("[ERROR] Error during dYdX positions update:", dydx_result)
            return jsonify(dydx_result), 500
        op_logger.log("dYdX Updated", source=source, operation_type="dYdX Updated")
    except Exception as e:
    ...
    # Step 4: Trigger price update
    try:
        logger.debug("Step 4: Triggering price update...")
    ...
    # Step 5: Manual alert check
    try:
        logger.debug("Step 5: Performing manual alert check via alert_manager.check_alerts()...")
    ...
    # Step 6: Update last update timestamps
    try:
        logger.debug("Step 6: Updating last update timestamps...")
        now = datetime.now()
    ...
    # Step 7: Record positions snapshot
    try:
        logger.debug("Step 7: Recording positions snapshot...")
    ...
    # Step 8: Emit SocketIO event for data update
    try:
        logger.debug("Step 8: Emitting SocketIO event for data update...")
        socketio_inst = get_socketio()
        if socketio_inst:
            socketio_inst.emit('data_updated', {
                'message': f"Jupiter & dYdX positions + Prices updated successfully by {source}!",
    ...
    # Step 9: Log updated totals
    try:
        logger.debug("Step 9: Logging updated totals...")
        updated_totals = dl.get_balance_vars()
        pst_timestamp = _convert_iso_to_pst(now.isoformat())
        print(f"[DEBUG] Update Complete: Totals = {updated_totals} at {pst_timestamp}")
        logger.debug(f"Update Complete: Totals = {updated_totals} at {pst_timestamp}")
    except Exception as e:
        logger.error(f"Exception logging updated totals: {e}", exc_info=True)
        print(f"[ERROR] Exception logging updated totals: {e}")

    # Step 10: Log operation with OperationsLogger
    try:
        logger.debug("Step 10: Logging operation with OperationsLogger...")
    ...
    logger.debug("update_positions route completed successfully.")
    print("[DEBUG] update_positions route completed successfully.")
